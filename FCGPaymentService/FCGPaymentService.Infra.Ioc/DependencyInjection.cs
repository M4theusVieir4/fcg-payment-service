...
using FCGPaymentService.Infra.Data.ElasticSearch;
...
public static class DependencyInjection
{
    public static void AddInfrastructure(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await ElasticsearchIndexSetup.CreateIndicesAsync(client);
                Console.WriteLine("Índices Elasticsearch criados com sucesso!");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Erro ao criar índices Elasticsearch: {ex.Message}");
            }
        });
    }
}
